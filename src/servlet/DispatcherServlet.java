...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("/register.do")) {
			...
			if (list.isEmpty()) {
				service.userAdd(vo);
				out.println("<script>alert('회원가입이 완료되었습니다.'); location.href='movie.do';</script>");
				out.flush();
				out.close();
			} else {
				out.println("<script>alert('이미 등록된 이메일입니다.'); location.href='movie.do';</script>");
				out.flush();
				out.close();
			}
		...
		if (action.equals("/addtv.do")) {
			...
			response.sendRedirect("tvshow.do");
		...
		if (action.equals("/search.do")) {
			...
			if (keyword.isEmpty()) {
				...
				out.println("<script>alert('한 글자 이상 입력해주세요.'); location.href='movie.do';</script>");
				out.flush();
				out.close();
			} else {
				UserDAO_Mariadb dao = new UserDAO_Mariadb();
